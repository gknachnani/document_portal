...
class DocumentIngestor:
    ...
    def __init__(self, temp_dir:str = "data/multi_doc_chat", faiss_dir: str = "faiss_index", session_id: str | None = None):
        try:
            ...
            self.session_id_faiss_dir = self.faiss_dir / self.session_id
            self.session_temp_dir.mkdir(parents=True, exist_ok=True)
            self.session_id_faiss_dir.mkdir(parents=True, exist_ok=True)
            ...
            self.log.info(
                ...
                faiss_path=str(self.session_id_faiss_dir)
                )
        except Exception as e:
            self.log.error("Failed to initialize DocumentIngestor",error=str(e))
            raise DocumentPortalException("Initialization error in DocumentIngestor",sys) from e
    ...
    def ingest_files(self, uploaded_files):
        try:
            ...
            raise DocumentPortalException("Ingestion error in DocumentIngestor",sys) from e
    ...
    def _create_retriever(self,documents):
        try:
            ...
            raise DocumentPortalException("Retriever error in DocumentIngestor",sys) from e