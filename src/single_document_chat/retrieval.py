...
class ConversationalRAG:
    def __init__(self, session_id: str, retriever) -> None:
        try:
            ...
            raise DocumentPortalException("Failed to initialize ConversationalRAG",sys) from e
    ...
    def load_retriever_from_faiss(self):
        try:
            ...
            raise DocumentPortalException("Error loading retriever from FAISS",sys) from e
        
    def invoke(self):
        try:
            pass
        except Exception as e:
            self.log.error("Failed to invokce conversational RAG",error=str(e), session_id=self.session_id)
            raise DocumentPortalException("Failed to invoke RAG chain",sys) from e
